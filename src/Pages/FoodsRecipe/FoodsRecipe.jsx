...
function FoodsRecipe({ match }) {
  ...
  const findStartedRecipeInStorage = () => {
    const recepiesInProgress = JSON.parse(
  ...
  const startRecipe = () => {
    const recipesInStorage = JSON.parse(
      localStorage.getItem("inProgressRecipes")
    );
  ...
  const favoriteRecipe = () => {
    ...
    localStorage.setItem(
      "favoriteRecipes",
      JSON.stringify(newFavoriteFoodRecipes)
    );
  ...
  const unfavoriteRecipe = () => {
    ...
    const newFavorites = favoriteRecipes.filter(
      (element) => element.id !== details.meals[0].idMeal
    );
  ...
  return (
    <div>
      {details.meals &&
        arrayIngredients.filterIngredients &&
        recommend.length > 0 && (
          <div className={styles.container}>
            ...
            <div className={styles.title_container}>
              ...
              <button
                ...
                onClick={() => {
                  copy(
                    `https://humbertodutra.github.io/${history.location.pathname}`
                  );
            ...
            <ul className={styles.container_foods}>
              {arrayIngredients.filterIngredients.map((elem, i) => (
                <li key={elem} data-testid={`${i}-ingredient-name-and-measure`}>
                  {elem[1]}{" "}
